...
import com.izzancode.kompoengdomba.databinding.ActivityDetailKematianKandangkuBinding
...
class detail_kematian_titip_angon : AppCompatActivity() {
    private  val binding by lazy { ActivityDetailKematianKandangkuBinding.inflate(layoutInflater) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dialog = progressDialog(this@detail_kematian_titip_angon)
    ...
    private  fun tampil(){
        lifecycleScope.launch {
            dialog.show()
            try {
                val authResponse = vm.detail_kematian(intent.getStringExtra("id").toString())
                if (authResponse.body()!!.success == 1) {
                    dialog.dismiss()
                    val result =authResponse.body()!!.data
                    var foto = foto_kambing +result.fv_image
                    Glide.with(this@detail_kematian_titip_angon)
                        .load(foto)
                    ...
                    binding.tvKeterangan.text  = result.fv_preventivemeasure
                    binding.tvKeteranganTanggal.text  = result.fd_deathdate
                    ...
                    binding.ivGambar.setOnClickListener {
                        startActivity(Intent(this@detail_kematian_titip_angon,com.izzancode.kompoengdomba.view.foto::class.java).putExtra("foto", foto_kambing +result.fv_image))
                    }
                } else {
                    dialog.dismiss()
                    Toast.makeText(
                        this@detail_kematian_titip_angon,
