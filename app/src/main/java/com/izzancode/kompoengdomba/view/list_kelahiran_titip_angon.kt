...
import com.izzancode.kompoengdomba.adapter.list_kelahiran_titip_angonAdapter
...
class list_kelahiran_titip_angon : AppCompatActivity() {
    ...
    private var rKandangkuAdapter : list_kelahiran_titip_angonAdapter? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(binding.root)
        vm = ViewModelProvider(this).get(titipangonViewModel::class.java)
        dialog = progressDialog(this@list_kelahiran_titip_angon)
    ...
    private fun dialog() {
        val dialog = Dialog(this@list_kelahiran_titip_angon)
        ...
        x.btKirim.setOnClickListener {
            if (tgl_awal == "" && tgl_akhir == "") {
                Toast.makeText(
                    this@list_kelahiran_titip_angon,
    ...
    private fun tampil() {
        ...
        lifecycleScope.launch {
            try {
                val authResponse = vm.list_kelahiran(this@list_kelahiran_titip_angon)
                if (authResponse.body()!!.success == 1) {
                    dialog.dismiss()
                    val result = authResponse.body()!!.data
                    rKandangkuAdapter = list_kelahiran_titip_angonAdapter(this@list_kelahiran_titip_angon, result)
                    if (this@list_kelahiran_titip_angon.resources.getConfiguration().orientation === Configuration.ORIENTATION_PORTRAIT) {
