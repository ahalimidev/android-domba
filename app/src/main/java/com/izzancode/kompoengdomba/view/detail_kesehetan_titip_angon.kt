...
import com.izzancode.kompoengdomba.adapter.detail_kesehatan_kandangkuAdapter
import com.izzancode.kompoengdomba.adapter.detail_kesehatan_titip_angonAdapter
import com.izzancode.kompoengdomba.databinding.ActivityDetailKesehetanKandangkuBinding
...
class detail_kesehetan_titip_angon : AppCompatActivity() {
    private  val binding by lazy { ActivityDetailKesehetanKandangkuBinding.inflate(layoutInflater) }
    lateinit var  vm : titipangonViewModel
    lateinit var dialog : Dialog
    private var rKandangkuAdapter : detail_kesehatan_titip_angonAdapter? = null
    var tgl_awal : String = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dialog = progressDialog(this@detail_kesehetan_titip_angon)
    ...
    private fun dialog() {
        val dialog = Dialog(this@detail_kesehetan_titip_angon)
        ...
        x.btKirim.setOnClickListener {
            if (tgl_awal == "" && tgl_akhir == "") {
                Toast.makeText(
                    this@detail_kesehetan_titip_angon,
                    "Pilih periode Kesehatan",
                    Toast.LENGTH_LONG
                ).show()
                dialog.dismiss()
            } else {
    ...
    private fun tampil (){
        ...
        lifecycleScope.launch {
            try {
                val authResponse = vm.detail_kesehatan(intent.getStringExtra("id").toString())
                if (authResponse.body()!!.success == 1) {
                    ...
                    Glide.with(this@detail_kesehetan_titip_angon)
                        .load(foto)
                    ...
                    val linearLayoutManager = LinearLayoutManager(this@detail_kesehetan_titip_angon, LinearLayoutManager.VERTICAL, false)
                    linearLayoutManager.scrollToPositionWithOffset(0, 0)
                    rKandangkuAdapter = detail_kesehatan_titip_angonAdapter(this@detail_kesehetan_titip_angon,result_all)
                    ...
                    binding.ivGambar.setOnClickListener {
                        startActivity(Intent(this@detail_kesehetan_titip_angon,com.izzancode.kompoengdomba.view.foto::class.java).putExtra("foto", foto_kambing +result_one.fv_image))
                    }
                }else{
                    dialog.dismiss()
    ...
    private fun tampil_filter(awal: String, akhir: String, kondisi: String) {
        ...
        lifecycleScope.launch {
            try {
                val authResponse = vm.filter_kesehatan(
