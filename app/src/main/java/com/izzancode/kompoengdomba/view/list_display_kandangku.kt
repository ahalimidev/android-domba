...
class list_display_kandangku : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dialog = progressDialog(this@list_display_kandangku)
    ...
    private fun tampil (status : String){
        ...
        lifecycleScope.launch {
            try {
                val authResponse = vm.list_display(intent.getStringExtra("fn_kavid").toString(),status)
                if (authResponse.body()!!.success == 1) {
                    dialog.dismiss()
                    val result = authResponse.body()!!.data
                    rKandangkuAdapter = list_display_titip_angonAdapter(this@list_display_kandangku,result)
                    if (this@list_display_kandangku.resources.getConfiguration().orientation === Configuration.ORIENTATION_PORTRAIT) {
                        binding.rvTampil.setLayoutManager(GridLayoutManager(this@list_display_kandangku, 2))
                    } else {
                        binding.rvTampil.setLayoutManager(GridLayoutManager(this@list_display_kandangku, 4))
                    }
